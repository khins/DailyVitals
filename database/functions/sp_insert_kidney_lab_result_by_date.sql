...
BEGIN
    INSERT INTO kidney_lab_result (
        person_id, result_month, albumin, npcr, potassium, wktv, calcium,
        phosphorus, ipth, hemoglobin, glucose, cholesterol, triglycerides,
        bun, creatinine, notes
    )
    VALUES (
        ...
        p_triglycerides, p_bun, p_creatinine, p_notes
    )
    RETURNING kidney_lab_result_id INTO v_id;

    INSERT INTO data_entry_log (
        table_name, record_id, action_type, entered_by, change_details
    )
    VALUES (
        'kidney_lab_result', v_id, 'INSERT', p_entered_by,
        jsonb_build_object(
